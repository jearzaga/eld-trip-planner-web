...
export function SummaryCards({ trip }: { trip: TripPlan }) {
  ...
  const items = [
    ...
    [
      'Arrival',
      formatDateTime(summary.arrive_at, summary.home_timezone),
      'summary-arrival',
    ],
  ...
  return (
    <section aria-labelledby="trip-summary-title">
      <h2 id="trip-summary-title" className="mb-3 text-lg font-semibold">
        Trip summary
      </h2>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {items.map(([label, value, testId]) => (
          <Card key={testId} size="sm">
            ...
            <CardContent>
              <p data-testid={testId} className="text-base font-semibold">
                {value}
              </p>
            </CardContent>
          </Card>
