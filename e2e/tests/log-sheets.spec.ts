...
test.describe('Daily log sheets', () => {
  ...
  test.fixme('AC-20: one log sheet appears for every calendar day in the trip', async ({
    page,
  }) => {
  ...
  test.fixme('AC-21: each sheet mirrors the complete Drivers Daily Log layout', async ({
    page,
  }) => {
  ...
  test.fixme('AC-22: the duty line is continuous through every status change', async ({ page }) => {
    const dutyLine = new LogSheetsPage(page).sheet(1).getByTestId('duty-line');

    await expect(dutyLine).toHaveAttribute('d', /^M.+[HV].+[HV].+/);
  });

  test.fixme('AC-23: status totals sum to twenty-four hours on every sheet', async ({ page }) => {
    const logs = new LogSheetsPage(page);
  ...
  test.fixme('AC-24: log headers include route, mileage, vehicle, and carrier details', async ({
    page,
  }) => {
    const sheet = new LogSheetsPage(page).sheet(1);
  ...
  test.fixme('AC-25: every duty-status change has a City, ST remark and grid marker', async ({
    page,
  }) => {
    const sheet = new LogSheetsPage(page).sheet(1);
  ...
  test.fixme('AC-26: recap values and restart notes are rendered from the trip plan', async ({
    page,
  }) => {
    const sheet = new LogSheetsPage(page).sheet(1);
  ...
  test.fixme('AC-27: drivers can page through and print one sheet per page', async ({ page }) => {
    const logs = new LogSheetsPage(page);
  ...
  test.fixme('SC-2 Day 1 matches the approved visual reference', async ({ page }) => {
    const sheet = new LogSheetsPage(page).sheet(1);
